CREATE     OR REPLACE PACKAGE BODY plex IS
  ...
  TYPE t_row_queries IS RECORD ( query                       VARCHAR2(32767),
  file_name                   VARCHAR2(256),
  max_rows                    NUMBER DEFAULT 100000 );
  TYPE t_tab_queries IS
    TABLE OF t_row_queries INDEX BY PLS_INTEGER;
  ...
  PROCEDURE util_temp_clob_reset
    IS
  BEGIN
    g_temp_clob            := NULL;
    g_temp_varchar_cache   := NULL;
  ...
  PROCEDURE util_temp_clob_flush_cache
    IS
  BEGIN
    IF
      g_temp_varchar_cache IS NOT NULL
    THEN
      IF
        g_temp_clob IS NULL
      THEN
  ...
  PROCEDURE util_temp_clob_append (
  ...
  BEGIN
    l_mail_id   := apex_mail.send(
      p_to     => g_channel_apex_mail,
      p_from   => nvl(
        g_channel_apex_mail_from,
        g_channel_apex_mail
      ),
      p_subj   => g_file_name,
      p_body   => g_file_name
  ...
  PROCEDURE util_deliver_apex_download
    IS
  BEGIN
    htp.flush;
    owa_util.mime_header(
      ccontent_type   => g_file_mime_type,
      bclose_header   => false,
      ccharset        => 'UTF-8'
    );
  ...
  PROCEDURE util_deliver_to_channels
    IS
  BEGIN
    apex_zip.finish(p_zipped_blob   => g_file_blob);
    IF
      g_channel_apex_mail IS NOT NULL
    THEN
      util_deliver_apex_mail;
    END IF;
    IF
      g_channel_apex_download
    THEN
  ...
  PROCEDURE util_query_to_csv (
    ...
    PROCEDURE local_temp_clob_append
      IS
    BEGIN
      util_temp_clob_append(
        CASE
          WHEN instr(
            nvl(
              l_buffer,
  ...
  PROCEDURE apex_backapp (
    ...
    PROCEDURE find_owner IS
      CURSOR cur_owner IS SELECT owner
                            FROM apex_applications t
                           WHERE t.application_id = p_app_id;

    BEGIN
      OPEN cur_owner;
      FETCH cur_owner   INTO l_owner;
      CLOSE cur_owner;
      IF
        l_owner IS NULL
      THEN
    ...
    BEGIN
      ...
      FOR i IN (
        ...
        util_temp_clob_append(dbms_metadata.get_ddl(
          object_type   => 'TABLE',
          name          => i.table_name,
          schema        => i.owner
        ) );
        ...
        THEN
          util_temp_clob_reset;
          util_query_to_csv(
            p_query      => util_get_query(p_table_name   => i.table_name),
  ...
  BEGIN
    util_init_export_method(
      p_file_name   => regexp_replace(
        srcstr       => p_zip_file_name,
        pattern      => '\.zip$',
        replacestr   => NULL,
        position     => 1,
        occurrence   => 0,
    ...
    FOR i IN g_queries_array.first..g_queries_array.last LOOP
      util_temp_clob_reset;
      util_query_to_csv(
        p_query      => g_queries_array(i).query,
      ...
      apex_zip.add_file(
        p_zipped_blob   => g_file_blob,
        p_file_name     => regexp_replace(
          srcstr       => g_queries_array(i).file_name,
          pattern      => '\.csv$',
          replacestr   => NULL,
          position     => 1,
          occurrence   => 0,
