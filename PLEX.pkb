...
^'
    ...
    -- file one
      ...
      util_g_clob_append(
        '-- DO NOT TOUCH THIS FILE or set the PLEX.BackApp parameter p_include_object_ddl to false - otherwise your changes would be overwritten on next PLEX.BackApp call.' || c_lf ||
        '-- It is recommended to export your object ddl only ones on initial repository creation and then start to use the "files first approach".' || c_lf || 
        c_lf || 
        'set define off verify off feedback off' || c_lf || 
        'whenever sqlerror exit sql.sqlcode rollback' || c_lf || 
        c_lf);

      util_g_clob_append(
        'prompt --install_backend_generated_by_plex' || c_lf || 
        c_lf);
...
If you need to do modifications
for the install process then have a look at the following templates - they call
the generated files and you can do your own stuff before or after the calls.
