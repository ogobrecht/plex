...
-- Script generated by PLEX version 2.2.0.4 - more infos here: https://github.com/ogobrecht/plex
...
to_clob(q'^CREATE OR REPLACE PACKAGE BODY plex IS
...
FUNCTION util_b^')
|| to_clob(q'^ool_to_string (p_bool IN BOOLEAN) RETURN VARCHAR2;
  ...
  LOOP
    ...
    dbms_lob.append(
      ...
      utl_raw.concat(
        ...
        util_zip_little_endian(t_offs - 1), -- relative offset of ^')
|| to_clob(q'^local file header
        dbms_lob.substr(
          p_zipped_blob,
  ...
  IF p_query IS NOT NULL THEN
    ...
    LOOP
      ...
      FOR i IN 1..v_col_count LOOP
       ...
       ^')
|| to_clob(q'|   v_buffer_clob := v_buffer_xmltype.getclobval();
          IF length(v_buffer_clob) <= c_vc2_max_size THEN
            v_buffer_varchar2 := substr(v_buffer_clob, 1, c_vc2_max_size);
            escape_varchar2_buffer_for_csv;
            util_clob_append(v_buffer_varchar2);
...
--  v_buffer_long              LONG;
  ...
  BEGIN
    ...
    --
   |')
|| to_clob(q'^ elsif instr(p_string, '|''') = 0 then
      return 'q''|' || p_string || '|''';
    --
    elsif instr(p_string, '!''') = 0 then
      return 'q''!' || p_string || '!''';
...
FUNCTION backapp (
  ...
  p_app_notifications           IN BOOLEAN  DEFAULT^')
|| to_clob(q'^ false,
...
RETURN tab_export_files IS
  ...
  BEGIN
    ...
    v_apex_files := apex_export.get_application(
      ...
      p_with_pkg_^')
|| to_clob(q'^app_mapping    => p_app_pkg_app_mapping,
  ...
  BEGIN
    ...
    v_query := replace(
      ...
      nvl(util_join(v_express^')
|| to_clob(q'`ion_table, ' and '), '1 = 1'));
    $if $$debug_on $then
...
WITH t AS (
  SELECT CASE object_type
          ...
          `')
|| to_clob(q'` WHEN 'EVALUATION CONTEXT'   THEN 'PROCOBJ'
...
^'  ;
   ...
   `')
|| to_clob(q'` util_setup_dbms_metadata;
...
'   );
    ...
    FOR i IN 1..v_ddl_files.ref_constraints_.count LOOP
      util_clob_append(util_get_script_li`')
|| to_clob(q'^ne(v_ddl_files.ref_constraints_(i)));
    END LOOP;
...
SELECT table_name,
       (SELECT LISTAGG(column_name, ', ') WITHIN GROUP(ORDER BY position)
         ...
         WHERE constraint_name = (SELECT constraint_name
                     ...
                     ^')
|| to_clob(q'`                AND c.table_name = t.table_name)
       ) AS pk_columns
  FROM user_tables t
...
^'    ;
      v_fil`')
|| to_clob(q'`e_path := 'plex_README.md';
      util_log_start(v_file_path);
...
if /i %areyousure% neq y goto END
set NLS_LANG=AMERICAN_AMERICA.UTF8
set /p "password=Please ente`')
|| to_clob(q'`r password for %app_schema% [default = oracle]: " || set "password=oracle"
echo This is the runlog for %scriptfile% on %app_schema%@%systemrole%(%connection%) > %logfile%
echo exit | sqlplus -S %app_schema%/%password%@%connection% ^
{{@}}%scriptfile% ^
%logfile% ^
...
BEGIN
  ...
  END IF;
END plex;
/^'),utl_raw.cast_to_raw('Dummy blob for API method get_a_row: xUdOxgOxwMkzwFXmmhwqCuBFEqMaezMTgIznRpYJCZtY'),'1915-12-04 00:00:00','2020-11-10 20:25:28.522977','2020-11-10 20:25:28.522983 +00:00','2020-11-10 21:25:28.522985','+01 02:03:04.000000','+01-02',
null)
select * from dual;
commit;
